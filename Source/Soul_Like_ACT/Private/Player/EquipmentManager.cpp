...
bool UEquipmentManager::SetJewelSocket(EEquipmentSlot EquipSlot, UPA_Jewel* InJewel, int32 SocketIndex,
									   UPA_Jewel*& OutJewel, bool bFirstEmptySlot)
{
	...
	const int32 ValidSocketIndex = bFirstEmptySlot
									   ? UBPFL_ItemMaker::GetFirstAvailableJewelSlot(Equipments[static_cast<int32>(
										   EquipSlot)])
									   : SocketIndex;

	const bool Result = UBPFL_ItemMaker::SetJewelSocket(this, static_cast<uint8>(EquipSlot), ValidSocketIndex, InJewel,
														OutJewel);
...
bool UEquipmentManager::SwapItemWithOtherInterface(uint8 FromSlot,
												   TScriptInterface<IInterface_InventoryManagement> ToComponent,
												   uint8 ToSlot)
{
	if (!ToComponent)
	{
...
void UEquipmentManager::UpdateWeaponActor(UPA_Weapon* WeaponData)
{
	...
	if (WeaponData)
	{
		...
		//spawn world actor
		CurrWeaponActor = GetWorld()->SpawnActor<AWeaponActor>(AWeaponActor::StaticClass(), FTransform::Identity,
															   SpawnParams);
		CurrWeaponActor->SetupWeaponSpecs(WeaponData);

		//Attach to SK
		FName SocketName;
		UBPFL_Equipment::StanceToSocketName(WeaponData->DefaultStance, SocketName);
